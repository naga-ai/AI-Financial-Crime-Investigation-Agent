...
def trace_triage(func: Callable) -> Callable:
    """Decorator to trace triage classification calls."""
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            span.output_data = {
                ...
                "should_investigate": getattr(result, "should_investigate", False),
            ...
            span.cost_usd = estimate_cost(triage_calls=1)
...
def trace_tool(tool_name: str) -> Callable:
    """Decorator factory to trace investigation tool calls."""
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                result = func(*args, **kwargs)
                span.output_data = {"type": type(result).__name__, "size": len(str(result))}
                span.status = "ok"
            except Exception as e:
                span.status = f"error: {e}"
                raise
                ...
                span.end_time = time.time()
                span.duration_ms = (span.end_time - span.start_time) * 1000
                span.cost_usd = estimate_cost(tool_calls=1)
...
def trace_investigation(func: Callable) -> Callable:
    """Decorator to trace a full investigation run."""
    @functools.wraps(func)
    def wrapper(*args, **kwargs):
        alert_id = kwargs.get("alert_id", args[0] if args else "unknown")
        client_id = kwargs.get("client_id", args[1] if len(args) > 1 else "unknown")
        ...
        try:
            result = func(*args, **kwargs)
            trace.metadata["risk_score"] = result.get("risk_score", 0) if isinstance(result, dict) else 0
            trace.metadata["recommended_action"] = result.get("recommended_action", "unknown") if isinstance(result, dict) else "unknown"
        except Exception as e:
...
def trace_report_generation(func: Callable) -> Callable:
    """Decorator to trace STR report generation."""
    @functools.wraps(func)
    def wrapper(*args, **kwargs):
        t0 = time.time()
        span = TraceSpan(
        ...
        try:
            result = func(*args, **kwargs)
            span.output_data = {
                "report_id": getattr(result, "report_id", ""),
                "risk_score": getattr(result, "risk_score", 0),
                "recommended_filing": getattr(result, "recommended_filing", False),
                "narrative_length": len(getattr(result, "narrative", "")),
            }
            narrative_len = len(getattr(result, "narrative", ""))
            span.cost_usd = estimate_cost(
                input_tokens=2000,
                output_tokens=int(narrative_len / 4),
            )
            span.status = "ok"
        except Exception as e:
            span.status = f"error: {e}"
